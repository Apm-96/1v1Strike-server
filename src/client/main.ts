...
//Modificat per jo, no se esl que estic fent
// Funció per segrestar les tecles del navegador
function preventBrowserShortcuts() {
    window.addEventListener('keydown', (e: KeyboardEvent) => {
        // Bloquegem combinacions amb Ctrl o Alt que fan accions al navegador
        // 'w' (tancar), 's' (guardar), 'r' (recarregar), 't' (nova pestanya), 'n' (nova finestra)
        const keysToBlock = ['w', 's', 'r', 't', 'n', 'p', 'f', 'd', 'g'];
        const key = e.key.toLowerCase();

        if ((e.ctrlKey || e.altKey) && keysToBlock.includes(key)) {
            e.preventDefault();
            e.stopPropagation();
        }

        // Bloquejar tecles de funció (F5 és recarregar, F1 ajuda, etc.)
        if (['F1', 'F3', 'F5', 'F6', 'F7', 'F11'].includes(e.key)) {
            e.preventDefault();
            e.stopPropagation();
        }
    }, { capture: true });

    // Bloquejar el menú del botó dret (molt important per shooters)
    window.addEventListener('contextmenu', (e) => e.preventDefault(), false);
}

async function main() {
    //Modificat per jo, no se esl que estic fent, tab = 4 espais
