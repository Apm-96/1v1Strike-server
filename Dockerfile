...
RUN git clone --branch merged --single-branch https://github.com/yohimik/xash3d-fwgs . \
...
# Configurem Postfix per a "Internet Site" de forma automàtica (per lo del mail, borrar si ja no cal)
RUN echo "postfix postfix/main_mailer_type string 'Internet Site'" | debconf-set-selections && \
    echo "postfix postfix/mailname string localhost" | debconf-set-selections

# Copiem el teu valve.zip local a la carpeta public i script dels logs i del entrypoint
COPY ./docker/cs-web-server/valve.zip ./public/valve.zip
COPY /docker/cs-web-server/watch_logs.sh /xashds/watch_logs.sh
COPY /docker/cs-web-server/entrypoint.sh /xashds/entrypoint.sh
RUN chmod +x /xashds/watch_logs.sh /xashds/entrypoint.sh

# Descomprimim el zip, movem els mapes i netegem
RUN unzip ./public/valve.zip -d /tmp/extracted_valve \
    && mkdir -p ./cstrike/maps ./public/cstrike/maps \
    && cp -rn /tmp/extracted_valve/cstrike/maps/* ./cstrike/maps/ \
    && cp -rn /tmp/extracted_valve/cstrike/maps/* ./public/cstrike/maps/ \
    && rm -rf /tmp/extracted_valve
# Crear carpeta de logs
RUN mkdir -p /xashds/cstrike/logs \ && chown -R xashds:xashds /xashds/cstrike/logs
RUN mkdir -p /xashds/cstrike/logs /xashds/saved_logs \
    && chown -R xashds:xashds /xashds
USER xashds
